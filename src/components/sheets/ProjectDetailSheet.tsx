...
interface Project { name: string; health: Health; visits: number; sales: number; totalLeads?: number; }
      ...
      <SheetContent className="sm:max-w-md bg-card border-border overflow-y-auto">
        <SheetHeader className="pb-4">
          <div className="flex items-center gap-2">
            <Badge variant="outline" className={`text-xs font-mono ${h.badge}`}>
              <span className={`h-2 w-2 rounded-full ${h.dot} mr-1.5`} />
              {h.label}
            </Badge>
          </div>
          ...
          <SheetDescription className="text-sm text-muted-foreground">Данные из CRM</SheetDescription>
        </SheetHeader>

        <Separator className="bg-border" />

        <div className="py-4 space-y-3">
          <h3 className="text-xs font-semibold uppercase tracking-[0.1em] text-muted-foreground">Показатели</h3>
          <div className="grid grid-cols-3 gap-3">
            {project.totalLeads !== undefined && (
              <div className="rounded-lg border border-border bg-secondary/20 p-3">
                <div className="flex items-center gap-1.5 mb-1">
                  <Users className="h-3.5 w-3.5 text-muted-foreground" />
                  <span className="text-xs text-muted-foreground uppercase tracking-wider">Лиды</span>
                </div>
            ...
            <div className="rounded-lg border border-border bg-secondary/20 p-3">
              <div className="flex items-center gap-1.5 mb-1">
                <Eye className="h-3.5 w-3.5 text-[hsl(var(--status-ai))]" />
                <span className="text-xs text-muted-foreground uppercase tracking-wider">Визиты</span>
              </div>
              <p className="text-lg font-bold font-mono tabular-nums">{project.visits}</p>
            </div>
            <div className="rounded-lg border border-border bg-secondary/20 p-3">
              <div className="flex items-center gap-1.5 mb-1">
                <ShoppingCart className="h-3.5 w-3.5 text-[hsl(var(--status-good))]" />
                <span className="text-xs text-muted-foreground uppercase tracking-wider">Продажи</span>
              </div>
        ...
        <div className="py-4 space-y-2">
          ...
          <Button variant="outline" size="sm" className="text-sm border-border w-full"><FileText className="h-3.5 w-3.5 mr-1.5" />Сформировать отчёт</Button>
        </div>
      </SheetContent>
    </Sheet>
  );
