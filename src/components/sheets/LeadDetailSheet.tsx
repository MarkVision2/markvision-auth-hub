...
interface Lead { name: string; project: string; source: string; time: string; phone: string; assignedTo: "ai" | "manager"; }
interface Props { lead: Lead | null; open: boolean; onOpenChange: (open: boolean) => void; }
      ...
      <SheetContent className="sm:max-w-md bg-card border-border overflow-y-auto">
        <SheetHeader className="pb-4">
          <div className="flex items-center gap-2">
            {lead.assignedTo === "ai" ? (
              <Badge variant="outline" className="text-xs font-mono border-[hsl(var(--status-ai)/0.3)] bg-[hsl(var(--status-ai)/0.1)] text-[hsl(var(--status-ai))]">
                <Bot className="h-3 w-3 mr-1" />AI
              </Badge>
            ) : (
              <Badge variant="outline" className="text-xs font-mono border-border text-muted-foreground">
                <User className="h-3 w-3 mr-1" />Менеджер
              </Badge>
            )}
          </div>
          <SheetTitle className="text-base font-semibold">{lead.name}</SheetTitle>
          <SheetDescription className="text-sm text-muted-foreground">{lead.project} · {lead.source}</SheetDescription>
        </SheetHeader>
