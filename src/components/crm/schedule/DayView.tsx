...
export const DayView: React.FC<DayViewProps> = ({
    ...
    return (
        <div className="flex flex-col h-full bg-background select-none animate-in slide-in-from-right-4 duration-500">
            {/* Day Header Summary */}
            <div className="px-8 py-6 border-b border-border bg-card flex items-center justify-between z-20 shadow-sm">
                <div className="flex items-center gap-5">
                    <div className="h-16 w-16 rounded-[20px] bg-blue-500/10 flex items-center justify-center text-blue-400 shadow-sm shrink-0">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <span className="text-[11px] uppercase font-bold tracking-[0.15em] text-muted-foreground flex items-center gap-2">
                             <Clock className="h-3.5 w-3.5" /> Рабочий день: 08:00 – 18:00
                        </span>
                    </div>
                </div>

                <div className="flex items-center gap-8 bg-secondary border border-border px-6 py-3 rounded-2xl shadow-sm">
                    <div className="flex flex-col items-end">
                        <span className="text-2xl font-black text-blue-400 leading-none mb-1">2</span>
                        <span className="text-[10px] uppercase font-bold tracking-[0.15em] text-muted-foreground">Всего записей</span>
                    </div>
                    <div className="h-10 w-px bg-border" />
                    <div className="flex flex-col items-end">
                        <span className="text-2xl font-black text-emerald-500 leading-none mb-1">1</span>
                        <span className="text-[10px] uppercase font-bold tracking-[0.15em] text-muted-foreground">Завершено</span>
                    </div>
                </div>
            </div>
                        ...
                        return (
                            <div key={hour} className="group relative flex items-start gap-6">
                                ...
                                <div className="w-24 pt-6 flex flex-col items-end shrink-0">
                                    <span className="text-[13px] font-bold text-muted-foreground tracking-[0.05em] transition-colors">
                                        {hourStr}
                                    </span>
                                ...
                                <div className="flex-1 min-h-[70px] pb-4 relative">
                                    ...
                                    {appt ? (
                                        ...
                                        >
                                            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                                                <div className="space-y-5 flex-1 min-w-0">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <span className={cn(
                                                            ...
                                                            appt.status === "completed" ? "bg-emerald-500/20 text-emerald-300" : "bg-amber-500/20 text-amber-300"
                                                        )}>
                                                            {getStatusText(appt.status)}
                                                        </span>
                                                    </div>

                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 bg-black/20 p-4 rounded-xl border border-border/50">
                                                    ...
                                                    <div className="flex items-center gap-3 pt-1">
                                                        <Button variant="secondary" size="sm" className="h-9 rounded-xl text-[10px] font-bold uppercase tracking-[0.15em] bg-card border-border text-muted-foreground hover:bg-secondary shadow-sm transition-all">
                                                            Перейти в карту
                                                        </Button>
                                                        <Button variant="secondary" size="icon" className="h-9 w-9 rounded-xl bg-card border-border text-muted-foreground hover:bg-secondary shadow-sm transition-all">
                                                ...
                                                <div className="flex flex-col items-end gap-2 shrink-0">
                                                    <span className="text-[15px] font-bold opacity-70 bg-black/20 px-2.5 py-1 rounded-lg">{appt.time}</span>
                                                    <span className="text-[10px] font-bold uppercase tracking-[0.15em] opacity-70 bg-black/20 px-3 py-1.5 rounded-lg">
                                                        {appt.type}
                                                    </span>
                                                </div>
                                            </div>
                                        ...
                                        <div 
                                            ...
                                            className="w-full h-full min-h-[60px] rounded-[24px] border-2 border-dashed border-border flex items-center p-4 transition-all duration-300 hover:border-blue-400 hover:bg-accent group/empty cursor-pointer bg-card"
                                        >
                                            <div className="flex items-center gap-5 transition-transform duration-300 group-hover/empty:translate-x-3">
                                                <div className="h-12 w-12 rounded-xl bg-secondary flex items-center justify-center text-muted-foreground border border-border group-hover/empty:bg-blue-900/30 group-hover/empty:border-blue-800/50 group-hover/empty:text-blue-400 transition-colors shadow-sm">
                                                    <Plus className="h-5 w-5" />
                                                </div>
                                                <span className="text-[11px] uppercase font-bold tracking-[0.15em] text-muted-foreground group-hover/empty:text-blue-400 transition-colors">
