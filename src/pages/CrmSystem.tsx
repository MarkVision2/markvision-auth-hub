...
export default function CrmSystem() {
  ...
  const kpiCards = [
    {
      ...
      value: kpis.total,
      suffix: "",
      sub: `+${kpis.todayLeads} сегодня`,
      icon: Users,
      color: "primary" as const,
      trend: kpis.todayLeads > 0 ? "up" as const : "neutral" as const,
    },
    {
      title: "В работе",
      value: kpis.inWork,
      suffix: "",
      sub: `${kpis.noResponse} без ответа`,
      icon: Clock,
      color: "warning" as const,
      trend: kpis.noResponse > 3 ? "down" as const : "neutral" as const,
    },
    {
      title: "Конверсия",
      value: kpis.convRate,
      suffix: "%",
      sub: `${kpis.paidCount} оплат`,
      icon: TrendingUp,
      color: "good" as const,
      trend: kpis.convRate > 10 ? "up" as const : "neutral" as const,
    },
    {
      title: "Выручка",
      value: fmt(kpis.revenue),
      suffix: " ₸",
      sub: `Средний скор: ${kpis.avg}%`,
      icon: DollarSign,
      color: "primary" as const,
      trend: kpis.revenue > 0 ? "up" as const : "neutral" as const,
    },
  ];
  ...
  const colorMap = {
    ...
    warning: {
      bg: "bg-[hsl(var(--status-warning)/0.06)]",
      icon: "bg-[hsl(var(--status-warning)/0.15)] text-[hsl(var(--status-warning))]",
      text: "text-[hsl(var(--status-warning))]",
      glow: "shadow-[hsl(var(--status-warning)/0.05)]",
    },
    good: {
      bg: "bg-[hsl(var(--status-good)/0.06)]",
      icon: "bg-[hsl(var(--status-good)/0.15)] text-[hsl(var(--status-good))]",
      text: "text-[hsl(var(--status-good))]",
      glow: "shadow-[hsl(var(--status-good)/0.05)]",
    },
  };
